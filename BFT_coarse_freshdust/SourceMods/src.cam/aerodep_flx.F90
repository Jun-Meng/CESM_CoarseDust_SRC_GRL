...
module aerodep_flx
  ...
  integer, parameter, public :: N_BULK = 22   !JM from 14 to 22
  ...
  character(len=12), parameter :: bulk_names(N_BULK) = (/ &
       ...
       'DSTX05DD    ', 'DSTX06DD    ', 'DSTX07DD    ', 'DSTX08DD    ', &   !JM add more bins
       'DSTX01WD    ', 'DSTX02WD    ', 'DSTX03WD    ', 'DSTX04WD    ', &   
       'DSTX05WD    ', 'DSTX06WD    ', 'DSTX07WD    ', 'DSTX08WD    ' /)   !JM 
  ...
  integer :: idstdry1,idstdry2,idstdry3,idstdry4,idstdry5,idstdry6,idstdry7,idstdry8 !JM add 5-8 bins
  integer :: idstwet1,idstwet2,idstwet3,idstwet4,idstwet5,idstwet6,idstwet7,idstwet8 !JM add 5-8 bins
...
!-------------------------------------------------------------------
  subroutine aerodep_flx_init()
    ...
    if (modal_fluxes) then
       ...
       idstdry5  = index_bulk_map(11)  !JM
       idstdry6  = index_bulk_map(12)
       idstdry7  = index_bulk_map(13)
       idstdry8  = index_bulk_map(14)
       idstwet1  = index_bulk_map(15)
       idstwet2  = index_bulk_map(16)
       idstwet3  = index_bulk_map(17)
       idstwet4  = index_bulk_map(18)
       idstwet5  = index_bulk_map(19)   !JM
       idstwet6  = index_bulk_map(20)
       idstwet7  = index_bulk_map(21)
       idstwet8  = index_bulk_map(22)
...
!-------------------------------------------------------------------
  subroutine set_bulk_fluxes( cam_out, ncol, lchnk )
    ...
    call set_fluxes( cam_out%dstdry5, idstdry5, ncol, lchnk )  !JM
    call set_fluxes( cam_out%dstdry6, idstdry6, ncol, lchnk )
    call set_fluxes( cam_out%dstdry7, idstdry7, ncol, lchnk )
    call set_fluxes( cam_out%dstdry8, idstdry8, ncol, lchnk )
    ...
    call set_fluxes( cam_out%dstwet5, idstwet5, ncol, lchnk )  !JM
    call set_fluxes( cam_out%dstwet6, idstwet6, ncol, lchnk )
    call set_fluxes( cam_out%dstwet7, idstwet7, ncol, lchnk )
    call set_fluxes( cam_out%dstwet8, idstwet8, ncol, lchnk )
