...
!    real(r8) :: flx_mss_hrz_slt_ttl !-variable no longer needed, -jfk
    ...
    !real(r8) :: fact_BFT(ndst)      !  ! [fraction unitless] !JM Factors from Brittle Fragmentation Theory -JM
!    
! constants
!
...
! initialize variables which get passed to the atmosphere
    ...
    do fp = 1,num_nolakep
       ...
       if (lnd_frc_mbl(p) > 0.0_r8) then
          ...
          if (wnd_frc_slt > wnd_frc_thr_slt) then
             ...
             flx_mss_vrt_dst_ttl(p) = Cd * mss_frc_cly_vld(c) * forc_rho(g) * ((wnd_frc_slt**2.0_r8 - wnd_frc_thr_slt**2.0_r8) / wnd_frc_thr_slt_std) * & 
                  (wnd_frc_slt / wnd_frc_thr_slt)**(Ca * (wnd_frc_thr_slt_std - wnd_frc_thr_slt_std_min) / wnd_frc_thr_slt_std_min)  ! the vertical dust flux -jfk
    ...
    !   end do
    !end do
    ! JM's proposed solution Jan. 6 2021
    do n = 1, ndst
       !do m = 1, dst_src_nbr
          do fp = 1,num_nolakep
             p = filter_nolakep(fp)
             if (lnd_frc_mbl(p) > 0.0_r8) then
                flx_mss_vrt_dst(p,n) = flx_mss_vrt_dst(p,n) +  fact_BFT(n) * flx_mss_vrt_dst_ttl(p)
             end if
          end do
       !end do
    end do
    ...
    !JM debuging totol emission
    !write(iulog,*) 'tot dust from CLM', SUM(flx_mss_vrt_dst_tot)
...
!
    ...
    ! JM 
    real(r8), pointer :: vlc_trb_5(:)   ! Turbulent deposition velocity 5
    real(r8), pointer :: vlc_trb_6(:)   ! Turbulent deposition velocity 6
    real(r8), pointer :: vlc_trb_7(:)   ! Turbulent deposition velocity 7
    real(r8), pointer :: vlc_trb_8(:)   ! Turbulent deposition velocity 8
!
! !REVISION HISTORY
! Created by Sam Levis
!
...
!------------------------------------------------------------------------
    ...
    ! JM 
    vlc_trb_5 => pdf%vlc_trb_5
    vlc_trb_6 => pdf%vlc_trb_6
    vlc_trb_7 => pdf%vlc_trb_7
    vlc_trb_8 => pdf%vlc_trb_8
    ...
    do p = lbp,ubp
       ...
       if (pwtgcell(p)>0._r8 .or. ityplun(l)==istice_mec) then
          ...
          !JM more bins 
          vlc_trb_5(p) = vlc_trb(p,5)
          vlc_trb_6(p) = vlc_trb(p,6)
          vlc_trb_7(p) = vlc_trb(p,7)
          vlc_trb_8(p) = vlc_trb(p,8)
       end if
    end do 
...
!------------------------------------------------------------------------
    ...
    if (ndst == 8) then     ! JM change from 4 to 8
       do n = 1, ndst
