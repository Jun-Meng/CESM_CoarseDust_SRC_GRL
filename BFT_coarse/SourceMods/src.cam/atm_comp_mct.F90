    ...
    do c=begchunk,endchunk
	  ...
	  cam_in(c)%landfrac(i)  =  x2a_a%rAttr(index_x2a_Sf_lfrac, ig)
	    ...
	    cam_in(c)%cflx(i,dust_ndx +3)  = -x2a_a%rAttr(index_x2a_Fall_flxdst4, ig)
            cam_in(c)%cflx(i,dust_ndx +4)  = -x2a_a%rAttr(index_x2a_Fall_flxdst5, ig)   !JM
            cam_in(c)%cflx(i,dust_ndx +5)  = -x2a_a%rAttr(index_x2a_Fall_flxdst6, ig)   !
            cam_in(c)%cflx(i,dust_ndx +6)  = -x2a_a%rAttr(index_x2a_Fall_flxdst7, ig)   !
            cam_in(c)%cflx(i,dust_ndx +7)  = -x2a_a%rAttr(index_x2a_Fall_flxdst8, ig)   !
#endif
          endif
  ...
  subroutine atm_export_mct( cam_out, a2x_a )
    ...
    do c=begchunk, endchunk
	  ...
	  a2x_a%rAttr(index_a2x_Sa_dens   ,ig) = cam_out(c)%rho(i)
          ...
          a2x_a%rAttr(index_a2x_Faxa_dstwet5,ig)  = cam_out(c)%dstwet5(i)  !JM
          a2x_a%rAttr(index_a2x_Faxa_dstdry5,ig)  = cam_out(c)%dstdry5(i)   !
          a2x_a%rAttr(index_a2x_Faxa_dstwet6,ig)  = cam_out(c)%dstwet6(i)  !
          a2x_a%rAttr(index_a2x_Faxa_dstdry6,ig)  = cam_out(c)%dstdry6(i)  !
          a2x_a%rAttr(index_a2x_Faxa_dstwet7,ig)  = cam_out(c)%dstwet7(i)  !
          a2x_a%rAttr(index_a2x_Faxa_dstdry7,ig)  = cam_out(c)%dstdry7(i)   !
          a2x_a%rAttr(index_a2x_Faxa_dstwet8,ig)  = cam_out(c)%dstwet8(i)   !
          a2x_a%rAttr(index_a2x_Faxa_dstdry8,ig)  = cam_out(c)%dstdry8(i)    !
