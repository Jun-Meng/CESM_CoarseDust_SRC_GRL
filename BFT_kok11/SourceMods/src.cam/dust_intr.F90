...
save
...
#else
  integer, parameter:: dust_number = 8  !JM 4->8
  integer, parameter:: ndst =8          !JM 4->8
#endif
  ...
  character(len=8), dimension(ncnst), parameter :: & ! constituent names
       cnst_names = (/'DST01', 'DST02', 'DST03', 'DST04', 'DST05', 'DST06','DST07','DST08'/) ! JM Dec 14 2020 add more bins
#endif
...
!===============================================================================
  ...
  subroutine dust_register_cnst( )
    ...
    ! JM add more bins 
    call cnst_add(cnst_names(5), mwght, one, zero, m )
    call cnst_add(cnst_names(6), mwght, one, zero, m )
    call cnst_add(cnst_names(7), mwght, one, zero, m )
    call cnst_add(cnst_names(8), mwght, one, zero, m )
    return
  ...
  subroutine dust_init_cnst(name, q, gcid)
    ...
    ! JM add more bins
       ...
       q = 0._r8
    else if ( name == cnst_names(6) ) then
       q = 0._r8
    else if ( name == cnst_names(7) ) then
       q = 0._r8
    else if ( name == cnst_names(8) ) then
       q = 0._r8
    end if
  ...
  subroutine dust_wet_intr (state, ptend, nstep, dt, lat, clat, cme, prain, &
    ...
    do m = 1, ndst
       ...
       !if (dust_has_wet_dep(m) ) then
          mm = ix_dust + m - 1
          !JM
          !write(iulog,*) 'Debugging: the dust index mm is:', mm
          scavt=0._r8
          !       write(iulog,*) 'wet dep removed for debugging'
          ptend%lq(mm) = .TRUE.
          ptend%name  = trim(ptend%name)//',dust'
          !JM testing why state%q is the same 
          !write(iulog,*) 'ptend name is', ptend%name
          sol_fact = 0.15_r8
          ...
          ! JM testing is state%q(:,:,mm) are the same for mm=20,21,22,23
          !call outfld( trim(cnst_name(mm))//'PP', state%q(:,:,mm), pcols, lchnk)
          ...
          if (.not. aerodep_flx_prescribed()) then
            ...
            ! JM add more bins
             case(5)
                do i = 1, ncol
                   cam_out%dstwet5(i) = max(-sflx(i), 0._r8)
                end do
             case(6)
                do i = 1, ncol
                   cam_out%dstwet6(i) = max(-sflx(i), 0._r8)
                end do
             case(7)
                do i = 1, ncol
                   cam_out%dstwet7(i) = max(-sflx(i), 0._r8)
                end do
             case(8)
                do i = 1, ncol
                   cam_out%dstwet8(i) = max(-sflx(i), 0._r8)
                end do
             end select
          endif
  ...
  subroutine dust_drydep_intr (state, ptend, wvflx, dt, lat, clat, &
    ...
    do m=1,ndst
       ...
       if (.not. aerodep_flx_prescribed()) then
          ...
          case(5)
             do i = 1, ncol
                cam_out%dstdry5(i) = max(sflx(i), 0._r8)
             end do
         case(6)
             do i = 1, ncol
                cam_out%dstdry6(i) = max(sflx(i), 0._r8)
             end do
          case(7)
             do i = 1, ncol
                cam_out%dstdry7(i) = max(sflx(i), 0._r8)
             end do
          case(8)
             do i = 1, ncol
                cam_out%dstdry8(i) = max(sflx(i), 0._r8)
             end do
          end select
       endif
    ...
    ptend%lq(ioff+1:ioff+8) = .TRUE.   !JM here should change it to 1:8, ioff = 15
...
! number and mass species
    ...
    !cflx(:ncol,dust_idx1())=cflx(:ncol,dust_idx1())*0.011_r8/0.032456_r8 !jfk, Jasper Kok, 8/27/18, based on Kok(2011)
    !cflx(:ncol,dust_idx1()+1)=cflx(:ncol,dust_idx1()+1)*0.087_r8/0.174216_r8
    !cflx(:ncol,dust_idx1()+2)=cflx(:ncol,dust_idx1()+2)*0.277_r8/0.4085517_r8
    !cflx(:ncol,dust_idx1()+3)=cflx(:ncol,dust_idx1()+3)*0.625_r8/0.384811_r8
    ! JM
    !cflx(:ncol,dust_idx1()+4)=cflx(:ncol,dust_idx1()+3)*0.625_r8/0.384811_r8  ! here need apply jasper's BFT new fraction factors
    !cflx(:ncol,dust_idx1()+5)=cflx(:ncol,dust_idx1()+3)*0.625_r8/0.384811_r8  ! here test if dst5-8 will be the same as dst4, since they are from the same
    !cflx(:ncol,dust_idx1()+6)=cflx(:ncol,dust_idx1()+3)*0.625_r8/0.384811_r8
    !cflx(:ncol,dust_idx1()+7)=cflx(:ncol,dust_idx1()+3)*0.625_r8/0.384811_r8
    !JM testing new proposed solution: only partition dust flux in CLM
    cflx(:ncol,dust_idx1())=cflx(:ncol,dust_idx1())
    cflx(:ncol,dust_idx1()+1)=cflx(:ncol,dust_idx1()+1)
    cflx(:ncol,dust_idx1()+2)=cflx(:ncol,dust_idx1()+2)
    cflx(:ncol,dust_idx1()+3)=cflx(:ncol,dust_idx1()+3)
    cflx(:ncol,dust_idx1()+4)=cflx(:ncol,dust_idx1()+4)
    cflx(:ncol,dust_idx1()+5)=cflx(:ncol,dust_idx1()+5)
    cflx(:ncol,dust_idx1()+6)=cflx(:ncol,dust_idx1()+6)
    cflx(:ncol,dust_idx1()+7)=cflx(:ncol,dust_idx1()+7)
#endif
...
#else
    do m=dust_idx1(),dust_idx1()+7     ! JM from 3 to 7 
       ! multiply by soil erodibility factor
       cflx(:ncol,m)=cflx(:ncol,m)*soil_erod_tmp(:ncol)/fact*1.15_r8
       sflx(:ncol)=sflx(:ncol)+cflx(:ncol,m)
#endif
  ...
  subroutine Dustini()
   ...
   ! JM add more dust bins
    real(r8) :: dmt_grd(9) =                  &     ![m] Particle diameter grid
         (/ 0.1e-6_r8, 1.0e-6_r8, 2.5e-6_r8, 5.0e-6_r8, 10.0e-6_r8, 14.0e-6_r8, 20.0e-6_r8, 28.0e-6_r8, 40.0e-6_r8 /)
#endif
...
#else
    if (dust_number == 8) then  ! JM change from 4 to 8
#endif
       do n = 1, dust_number
       ...
       ! JM add more bins: (maybe need more codes here)
