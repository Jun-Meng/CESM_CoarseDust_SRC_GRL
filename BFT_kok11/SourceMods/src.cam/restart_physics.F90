...
!
    ...
    type(var_desc_t) :: bcphidry_desc, bcphodry_desc, ocphidry_desc, ocphodry_desc, &
       dstdry1_desc, dstdry2_desc, dstdry3_desc, dstdry4_desc, dstdry5_desc, dstdry6_desc, dstdry7_desc, dstdry8_desc
  ...
  CONTAINS
    ...
    if ( .not. adiabatic .and. .not. ideal_phys )then
       ...
       ierr = pio_def_var(File, 'DSTDRY5',  pio_double, hdimids, dstdry5_desc) !JM
       ierr = pio_def_var(File, 'DSTDRY6',  pio_double, hdimids, dstdry6_desc)
       ierr = pio_def_var(File, 'DSTDRY7',  pio_double, hdimids, dstdry7_desc)
       ierr = pio_def_var(File, 'DSTDRY8',  pio_double, hdimids, dstdry8_desc)
...
! needs to be made because it is collective. 
	 ...
	 ii=0
         ...
         do i=begchunk,endchunk
            ...
            do j=1,pcols
               ii=ii+1
               if(j<=ncol) tmpfield(ii) = cam_out(i)%dstdry5(j)
            end do
         end do
         call pio_write_darray(File, dstdry5_desc, iodesc, tmpfield, ierr)
         ...
         do i=begchunk,endchunk
            ...
            do j=1,pcols
               ii=ii+1
               if(j<=ncol) tmpfield(ii) = cam_out(i)%dstdry6(j)
            end do
         end do
         call pio_write_darray(File, dstdry6_desc, iodesc, tmpfield, ierr)
         ...
         do i=begchunk,endchunk
            ...
            do j=1,pcols
               ii=ii+1
               if(j<=ncol) tmpfield(ii) = cam_out(i)%dstdry7(j)
            end do
         end do
         call pio_write_darray(File, dstdry7_desc, iodesc, tmpfield, ierr)
         ...
         do i=begchunk,endchunk
            ...
            do j=1,pcols
               ii=ii+1
               if(j<=ncol) tmpfield(ii) = cam_out(i)%dstdry8(j)
            end do
         end do
         call pio_write_darray(File, dstdry8_desc, iodesc, tmpfield, ierr)
         if (co2_transport()) then
            ii=0
            do i=begchunk,endchunk
               ncol = cam_out(i)%ncol
...
!#######################################################################
    ...
    subroutine read_restart_physics (File, cam_out,pbuf2d)
     ...
     if ( .not. adiabatic .and. .not. ideal_phys )then
        ...
        ierr = pio_inq_varid(File, 'DSTDRY5', vardesc)
        call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
        ii=0
        do c=begchunk,endchunk
           do i=1,pcols
              ii=ii+1
              cam_out(c)%dstdry5(i) = tmpfield(ii)
           end do
        end do

        ierr = pio_inq_varid(File, 'DSTDRY6', vardesc)
        call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
        ii=0
        do c=begchunk,endchunk
           do i=1,pcols
              ii=ii+1
              cam_out(c)%dstdry6(i) = tmpfield(ii)
           end do
        end do

        ierr = pio_inq_varid(File, 'DSTDRY7', vardesc)
        call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
        ii=0
        do c=begchunk,endchunk
           do i=1,pcols
              ii=ii+1
              cam_out(c)%dstdry7(i) = tmpfield(ii)
           end do
        end do

        ierr = pio_inq_varid(File, 'DSTDRY8', vardesc)
        call pio_read_darray(File, vardesc, iodesc, tmpfield, ierr)
        ii=0
        do c=begchunk,endchunk
           do i=1,pcols
              ii=ii+1
              cam_out(c)%dstdry8(i) = tmpfield(ii)
           end do
        end do
